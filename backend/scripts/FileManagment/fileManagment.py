...
def handle_uploaded_file(f):
    try:
        ...
        if f.name.split('.')[-1].lower() == 'pdb':
            models_num=len(PDBParser(PERMISSIVE=1,QUIET=True).get_structure("str",str(n.file.path)))
        if f.name.split('.')[-1].lower() == 'cif':
            models_num=len(FastMMCIFParser(QUIET=True).get_structure("str",str(n.file.path)))
        data_file.close()
    except Exception:
