...
class ResultComposerTest(TestCase):
    ...
    def test_get1JJP1_doComposeJSONResult_resultProperResult(self):
        ...
        httpretty.register_uri(
            httpretty.GET, str(WEBTETRADO_BACKEND_URL) + "/v1/result/" +
            "code_1jjp", body="".join(
                [i
                 for i in open(
                     str(BASE_DIR) + "/backend/tests/test_files/1jjp_1.json",
                     "r")]),)
    ...
    def test_get2HY91_doComposeJSONResult_resultProperResult(self):
        ...
        httpretty.register_uri(
            ...
            "code_2hy9", body="".join(
                [i
                 for i in open(
                     str(BASE_DIR) + "/backend/tests/test_files/2hy9_1.json",
                     "r")]),)
        entity = TetradoRequest()
        entity.complete_2d = False
        entity.no_reorder = False
        ...
        self.assertTrue(
            compare(
                data_file.name,
                str(BASE_DIR) +
                "/backend/tests/test_files/2hy9_1_cache.json",
                ['remove_date']
                )
        )
        data_file.close()
    ...
    def test_get6RS31_doComposeJSONResult_resultProperResult(self):
        ...
        httpretty.register_uri(
            ...
            "code_6rs3", body="".join(
                [i
                 for i in open(
                     str(BASE_DIR) + "/backend/tests/test_files/6rs3_1.json",
                     "r")]),)
        entity = TetradoRequest()
        entity.complete_2d = False
        entity.no_reorder = False
    ...
    def test_get6FC91_doComposeJSONResult_resultProperResult(self):
        ...
        httpretty.register_uri(
            ...
            "code_6fc9", body="".join(
                [i
                 for i in open(
                     str(BASE_DIR) + "/backend/tests/test_files/6fc9_1.json",
                     "r")]),)
        entity = TetradoRequest()
        entity.complete_2d = False
        entity.no_reorder = False
